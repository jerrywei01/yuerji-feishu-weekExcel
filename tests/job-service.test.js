...
test("JobService deletes uploaded xlsx after importing sheet", async () => {
  ...
  const feishuClient = {
    ...
    transferSheetOwner: async () => {
      calls.push(["transferSheetOwner", "abc123"]);
    }
  };
  ...
  assert.deepEqual(calls, [
    ["transferSheetOwner", "abc123"],
    ["deleteFile", "file_token_1"]
  ]);
});

test("JobService does not fail the whole run when uploaded xlsx cleanup fails", async () => {
  const calls = [];
  const feishuClient = {
    updateWeeklyRecord: async () => {},
    getWeeklyRecord: async () => ({
  ...
  const result = await service.run("rec_weekly_1");
